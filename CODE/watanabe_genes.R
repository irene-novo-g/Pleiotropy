...
data <- read_excel("Watanabe.xlsx", skip = 1, sheet = "ST 7")
...
colnames(data)[8:9] <- c("traits", "domains")
...
data$domains2 <- ifelse(data$domains >= 10, 10, data$domains)
...
ggplot() + 
  ...
  theme_bw() +
  theme(panel.border = element_rect(colour = "grey60"), text = element_text(face = "bold", size = 15), plot.title = element_text(size = 16, hjust = 0.5)) +
  scale_x_discrete(breaks = 1:10, labels = c("1", "2", "3", "4", "5", "6", "7", "8", "9", "\u2265 10"))
